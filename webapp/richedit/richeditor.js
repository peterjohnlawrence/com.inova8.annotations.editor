...
jQuery.sap.registerResourcePath('ckeditor', './ckeditor/ckeditor');
window.CKEDITOR_BASEPATH = './ckeditor/';
sap.ui.define(['jquery.sap.global', 'sap/ui/core/Control', 'richedit/CKEditorToolbar', 'ckeditor'],
	function(jQuery, Control, CKEditorToolbar, Editor) {
		...
		CKEditor.prototype.afterFirstRender = function() {
			...
			this.editor.on('blur', jQuery.proxy(this.onBlur, this));
		...
		CKEditor.prototype.onBlur = function() {
			var oldVal = this.getValue(),
				newVal = this.editor.getData();
			if (oldVal !== newVal) {
				this.setProperty('value', newVal, true); // suppress rerendering
			...
			while ((matchArray = pattern.exec(newVal)) !== null) {
				var match = matchArray[0].toLowerCase();
				if (keywordsObject.hasOwnProperty(match)) {
					keywordsObject[match].push(matchArray.index);
				} else {
					keywordsObject[match] = [];
					keywordsObject[match].push(matchArray.index);
				}
			}
